...
func getConfigFile() string {
	...
	return filepath.Join(tConfigPath, "login.json")
}
...
func handleLogInResponse(pLogInResponseDto LogInResponseDto) {
	...
	tLogInDto.AccessToken = pLogInResponseDto.AccessToken
	tLogInDto.RefreshToken = pLogInResponseDto.RefreshToken

	tEncoder := json.NewEncoder(tFh)
	tErr = tEncoder.Encode(&tLogInDto)
	if tErr != nil {
		ExitWithError("Error encoding LogInDto")
	}

	if tLogInDto.AccessToken != "" {
...
func getStoredLogin() (LogInDto, error) {
	...
	if _, tErr := os.Stat(tConfigFile); os.IsNotExist(tErr) {
		return tLogInDto, errors.New("NO LOGIN FOUND")
	} else {
		...
		tErr = tDecoder.Decode(&tLogInDto)
		if tErr != nil {
			ExitWithError("Error Decoding LogInDto")
		}
